...
public class Maze {
  ...
  public static int[][] createMaze(int numRows, int numCols) {
    ...
    for(int i = 0; i < numCols; i++){
      maze[0][i] = 1; //0 is free space, 1 is wall
      maze[numRows-1][i] = 1; //Start from 0 to length -1;
    }
  ...
  public static void divideMaze(int[][] maze, int startRow, int endRow,
    ...
    if (endRow - startRow < 2 && endCol - startCol < 2) {
      ...
      for(int i = startCol ; i <= endCol; i++){ //different forward statement
        maze[row][i] = 1;
      }
      ...
      if(noDoor != 0){
        maze[row][randOdd(startCol, col - 1)] = 0; //3 spaces for the corridors, we go with odd numbers
      }
      if(noDoor != 1){
  ...
  public static int randEven(int a, int b) {
    return 2 * randInt(a/2 + a%2 / b,2);//2);
    }
